...
#elif CONFIG_BSP_LCD_TYPE_JD9366_800_1280
    // create JD9366 control panel (integrated LCD+Touch)
    ESP_LOGI(TAG, "Install JD9366 LCD control panel");

#if CONFIG_BSP_LCD_COLOR_FORMAT_RGB888
    esp_lcd_dpi_panel_config_t dpi_config = JD9366_800_1280_PANEL_60HZ_DPI_CONFIG(LCD_COLOR_PIXEL_FORMAT_RGB888);
#else
...
#if CONFIG_BSP_LCD_COLOR_FORMAT_RGB888
    #define BSP_JD9366_COLOR_DEPTH (24)
#else
    #define BSP_JD9366_COLOR_DEPTH (16)
#endif

    const esp_lcd_panel_dev_config_t lcd_dev_config = {
        .reset_gpio_num = BSP_LCD_RST,
        .rgb_ele_order = BSP_LCD_COLOR_SPACE,
        .bits_per_pixel = BSP_JD9366_COLOR_DEPTH,
        .vendor_config = &vendor_config,
    };
    ESP_GOTO_ON_ERROR(esp_lcd_new_panel_jd9366(io, &lcd_dev_config, &disp_panel), err, TAG, "New LCD panel JD9366 failed");
    ESP_GOTO_ON_ERROR(esp_lcd_panel_reset(disp_panel), err, TAG, "LCD panel reset failed");
    ESP_GOTO_ON_ERROR(esp_lcd_panel_init(disp_panel), err, TAG, "LCD panel init failed");
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
...
#if CONFIG_BSP_TOUCH_GT911
    /* Configure for GT911 touch controller */
    esp_lcd_panel_io_i2c_config_t tp_io_config = ESP_LCD_TOUCH_IO_I2C_GT911_CONFIG();
    tp_io_config.scl_speed_hz = CONFIG_BSP_I2C_CLK_SPEED_HZ;
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_i2c(i2c_handle, &tp_io_config, &tp_io_handle), TAG, "Failed to create GT911 panel IO");
    ret = esp_lcd_touch_new_i2c_gt911(tp_io_handle, &tp_cfg, ret_touch);
    ESP_LOGI(TAG, "GT911 touch controller initialized");
    
#elif CONFIG_BSP_TOUCH_JD9366
    /* Configure for JD9366 touch controller */
    ...
    ret = esp_lcd_new_panel_io_i2c_v2((i2c_master_bus_handle_t)i2c_handle, &tp_io_config, &tp_io_handle);
#else
    ret = esp_lcd_new_panel_io_i2c_v2((esp_lcd_i2c_bus_handle_t)i2c_handle, &tp_io_config, &tp_io_handle);
#endif
    ESP_RETURN_ON_ERROR(ret, TAG, "Failed to create JD9366 panel IO");
    
    ret = esp_lcd_touch_new_i2c_jd9366(tp_io_handle, &tp_cfg, ret_touch);
    ESP_LOGI(TAG, "JD9366 touch controller initialized");
    
#else
    ESP_LOGE(TAG, "No touch controller selected in configuration");
...
#else
    ESP_LOGW(TAG, "Touch controller disabled in configuration");
    return ESP_ERR_NOT_SUPPORTED;
#endif
}
